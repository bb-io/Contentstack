    ...
    : AppInvocable(invocationContext)
{
    ...
    public async Task<GetEntryReferencesResponse> GetEntryReferences(
        [ActionParameter] EntryRequest input,
        [ActionParameter] ExcludeContentTypesRequest excludeContentTypes)
    {
        var endpoint = $"v3/content_types/{input.ContentTypeId}/entries/{input.ContentId}/references";
        var request = new ContentstackRequest(endpoint, Method.Get, Creds);
        var response = await Client.ExecuteWithErrorHandling<GetEntryReferencesResponse>(request);

        response.References = ContentTypeExclusionFilter
            .FilterReferences(response.References, excludeContentTypes.ExcludeContentTypeIds)
            .ToList();
        ...
        [ActionParameter] LocaleRequest locale)
    {
        ...
        {
            var referenced = EntryReferenceExtractor.ExtractReferencedEntriesWithContentTypes(
                entry, contentType, input.ExcludeContentTypeIds);
        ...
        {
            var allReferencedEntries = EntryReferenceExtractor.ExtractReferencedEntriesWithContentTypes(entry, contentType);

            response.ReferencedEntryUids = ContentTypeExclusionFilter.FilterReferencedEntryUids(
                EntryReferenceExtractor.ExtractReferencedEntryUids(entry, contentType),
                allReferencedEntries,
                input.ExcludeContentTypeIds);

            response.ReferencedEntries = ContentTypeExclusionFilter
                .FilterReferencedEntries(allReferencedEntries, input.ExcludeContentTypeIds)
